...
if ( ! function_exists( 'bright_autonomy_render_video' ) ) {
	function bright_autonomy_render_video( $video_field, $args = [] ) {
		$defaults = [
			'behavior'           => 'autoplay',
			'autoplay'           => true,
		...
		switch ( $video_source ) {
			case 'self_host':
				$video_html = bright_autonomy_render_self_hosted_video( $video_data, $args );
				break;
			case 'youtube':
				$video_html = bright_autonomy_render_youtube_video( $video_data, $args );
				break;
			case 'vimeo':
				$video_html = bright_autonomy_render_vimeo_video( $video_data, $args );
				break;
			case 'cdn':
				$video_html = bright_autonomy_render_cdn_video( $video_data, $args );
				break;
		}
		...
		// enqueued) globally — see bright_autonomy_scripts() — so pages without any video
		...
		wp_enqueue_style( 'bright-autonomy-video' );
		wp_enqueue_script( 'bright-autonomy-video-behaviors' );

		if ( 'onclick-popup' === $args['behavior'] ) {
			wp_enqueue_style( 'bright-autonomy-video-popup' );
			wp_enqueue_script( 'bright-autonomy-video-popup' );
		}
		...
		if ( 'onclick-popup' === $args['behavior'] ) {
			...
			$html .= '<button class="video-play-button" aria-label="' . esc_attr__( 'Play Video', 'bright-autonomy' ) . '">';
		...
		if ( 'autoplay' === $args['behavior'] && $args['controls'] && 'youtube' !== $video_source ) {
			if ( $args['autoplay_on_scroll'] || $args['autoplay'] ) {
				...
				$html .= '<button class="video-play-button low-power-play-btn" aria-label="' . esc_attr__( 'Play Video', 'bright-autonomy' ) . '">';
				$html .= '<svg xmlns="http://www.w3.org/2000/svg" width="22" height="26" viewBox="0 0 22 26" fill="none">';
			...
			if ( function_exists( 'bright_autonomy_render_video_autoplay_controls' ) ) {
				$html .= bright_autonomy_render_video_autoplay_controls( [ 'echo' => false ] );
			}
		}
...
if ( ! function_exists( 'bright_autonomy_render_self_hosted_video' ) ) {
	function bright_autonomy_render_self_hosted_video( $video_data, $args ) {
		...
		if ( $poster && isset( $poster['url'] ) ) {
			...
			$poster_url = esc_url( $poster['sizes']['bright-1600'] ?? $poster['url'] );
		}
		...
		if ( empty( $poster_url ) && $args['autoplay_on_scroll'] ) {
			...
			if ( $product && method_exists( $product, 'get_image_id' ) ) {
				...
				if ( $image_id ) {
					$poster_url = wp_get_attachment_image_url( $image_id, 'bright-1600' );
				}
			}
		}
		...
		$html .= esc_html__( 'Your browser does not support the video tag.', 'bright-autonomy' );
		$html .= '</video>';
...
if ( ! function_exists( 'bright_autonomy_render_youtube_video' ) ) {
	function bright_autonomy_render_youtube_video( $video_data, $args ) {
		...
		if ( 'onclick-popup' === $args['behavior'] ) {
			...
			$html = '<img src="' . esc_url( $thumbnail_url ) . '" alt="' . esc_attr__( 'Video Thumbnail', 'bright-autonomy' ) . '" ';
...
if ( ! function_exists( 'bright_autonomy_render_vimeo_video' ) ) {
	function bright_autonomy_render_vimeo_video( $video_data, $args ) {
...
if ( ! function_exists( 'bright_autonomy_render_cdn_video' ) ) {
	function bright_autonomy_render_cdn_video( $video_data, $args ) {
...
add_filter(
	...
	function ( $field ) {
		$spec = __( 'Upload a web-optimised MP4: 1080p max, under ~3&nbsp;MB, H.264, no audio track. Export a compressed / "for web" version — not the original 4K or camera file. Large videos noticeably slow the page (the file is served at full size; WordPress does not compress it).', 'bright-autonomy' );
