...
if ( ! function_exists( 'bright_autonomy_flexible_content' ) ) {
	function bright_autonomy_flexible_content( $field_name = 'cms', $post_id = null ) {
		...
		while ( have_rows( $field_name, $post_id ) ) {
			...
			$GLOBALS['bright_autonomy_previous_layout'] = $previous_layout;
			$GLOBALS['bright_autonomy_section_index']   = $section_index;
		...
		$GLOBALS['bright_autonomy_last_layout'] = $previous_layout;

		unset( $GLOBALS['bright_autonomy_previous_layout'] );
		unset( $GLOBALS['bright_autonomy_section_index'] );
	}
}

if ( ! function_exists( 'bright_autonomy_get_last_flexible_layout' ) ) {
	function bright_autonomy_get_last_flexible_layout( $field_name = 'cms', $post_id = null ) {
		if ( ! function_exists( 'have_rows' ) ) {
			return false;
		}
...
if ( ! function_exists( 'bright_autonomy_get_first_flexible_layout' ) ) {
	function bright_autonomy_get_first_flexible_layout( $field_name = 'cms', $post_id = null ) {
		if ( ! function_exists( 'have_rows' ) ) {
			return false;
		}
...
if ( ! function_exists( 'bright_autonomy_has_hero_first_section' ) ) {
	function bright_autonomy_has_hero_first_section( $field_name = 'cms', $post_id = null ) {
		// Blog page uses inner_hero from options, not flexible content
		if ( is_home() ) {
			return true;
		}

		$first_layout = bright_autonomy_get_first_flexible_layout( $field_name, $post_id );

		return in_array( $first_layout, [ 'hero_section', 'inner_hero' ], true );
	}
}

if ( ! function_exists( 'bright_autonomy_queried_cms_has_layout' ) ) {
	/**
	 ...
	 * @return bool
	 */
	function bright_autonomy_queried_cms_has_layout( $layouts, $post_id = null ) {
		if ( ! function_exists( 'have_rows' ) ) {
			return false;
		}
...
if ( ! function_exists( 'bright_autonomy_page_needs_slick' ) ) {
	/**
	 ...
	 * carousel markup opts in via the `bright_autonomy_page_needs_slick` filter (e.g.
	 * `return bright_autonomy_queried_cms_has_layout( [ 'my_carousel_layout' ] );`),
	 * or enqueues the registered `slick-carousel` handle at render time.
	 *
	 * @return bool
	 */
	function bright_autonomy_page_needs_slick() {
		return (bool) apply_filters( 'bright_autonomy_page_needs_slick', false );
	}
}
...
if ( ! function_exists( 'bright_autonomy_page_needs_contact_form' ) ) {
	 ...
	 * `bright_autonomy_page_needs_contact_form` filter (e.g. to also match a flexible
	 ...
	 * @return bool
	 */
	function bright_autonomy_page_needs_contact_form() {
		$needs   = false;
