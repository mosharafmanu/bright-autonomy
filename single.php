		...
		if ( have_posts() ) :
			while ( have_posts() ) :
				...
				if ( is_singular( 'post' ) ) :
					...
					if ( $mc_prev || $mc_next ) :
						?>
						<nav class="post-nav post-inner layout-padding mt-40 mt-lg-60" aria-label="<?php esc_attr_e( 'Article navigation', 'bright-autonomy' ); ?>">
							<?php if ( $mc_prev ) : ?>
								<a class="post-nav__link post-nav__link--prev" href="<?php echo esc_url( get_permalink( $mc_prev ) ); ?>" rel="prev">
									<span class="post-nav__label"><span aria-hidden="true">&larr;</span> <?php esc_html_e( 'Previous', 'bright-autonomy' ); ?></span>
							...
							<?php if ( $mc_next ) : ?>
								<a class="post-nav__link post-nav__link--next" href="<?php echo esc_url( get_permalink( $mc_next ) ); ?>" rel="next">
									<span class="post-nav__label"><?php esc_html_e( 'Next', 'bright-autonomy' ); ?> <span aria-hidden="true">&rarr;</span></span>
				...
				if ( function_exists( 'bright_autonomy_render_back_to_blogs_button' ) ) {
					bright_autonomy_render_back_to_blogs_button();
				}
				...
				if ( is_singular( 'post' ) && function_exists( 'bright_autonomy_render_post_card' ) ) :
					...
					if ( $mc_related_posts ) :
						...
						<section class="related-posts mc-container layout-padding pt-50 pb-50 pt-lg-90 pb-lg-90">
							<header class="related-posts__header">
								<h2 class="related-posts__title"><?php esc_html_e( 'Related Articles', 'bright-autonomy' ); ?></h2>
							</header>
							...
							<div class="blog-grid card-grid <?php echo esc_attr( $mc_related_cols ); ?> mt-30 mt-lg-50">
								<?php foreach ( $mc_related_posts as $mc_related_post ) : ?>
									<?php bright_autonomy_render_post_card( $mc_related_post->ID, [ 'variant' => $mc_related_variant ] ); ?>
								<?php endforeach; ?>
							</div>
						</section>
						<?php
