# Bright Autonomy — New Project Setup Checklist
...
- [ ] Copy `bright-autonomy/` theme folder directly into the new project's `wp-content/themes/`
      — ⚠ it must land flat as `wp-content/themes/<slug>/style.css`, not nested in a
      wrapper folder (e.g. `wp-content/themes/my-project/bright-autonomy/`), or WordPress
...
- [ ] Remove the copied `.git/` (`rm -rf .git`) before running `git init` — `cp -r` carries
      ...
      `mosharafmanu/bright-autonomy` repo instead of your project's
...
- [ ] Update `--bright-font-heading` and `--bright-font-body`
- [ ] Update Google Fonts URL in `functions.php`
- [ ] Review `--bright-container-max` and `--bright-section-padding-y` against the design grid
