# Bright Autonomy — Content Typography & Single Post
...
**CSS file:** `assets/css/bright-autonomy-design-style.css` — "CONTENT TYPOGRAPHY" section.
...
<article class="single-post">                            ← post_class() adds WP classes
  <div class="post-thumbnail">                           ← full-width, max-height: 34rem (18rem mobile)
    <picture> / <img class="post-thumbnail-image">       ← bright_autonomy_render_responsive_picture(), LCP
  </div>
...
**`assets/css/bright-autonomy-starter-style.css` — "Single Post" section:**
...
The featured image uses `bright_autonomy_render_responsive_picture()` with `fetchpriority: 'high'` and `lazy: false` — it is always above the fold on a single post. Falls back to `the_post_thumbnail('mc-1200')` if the helper is unavailable.
