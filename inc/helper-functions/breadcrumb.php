...
if ( ! function_exists( 'bright_autonomy_breadcrumb' ) ) {
	function bright_autonomy_breadcrumb( $layout_padding = false, $margin_top = 'mt-30', $margin_bottom = '' ) {
		if ( is_front_page() ) {
			return;
		}

		$classes = [ 'bright-autonomy-breadcrumb' ];
		...
		echo '<a href="' . esc_url( home_url( '/' ) ) . '">' . esc_html__( 'Home', 'bright-autonomy' ) . '</a>';
		...
		if ( is_home() ) {
			...
			if ( $posts_page ) {
				...
				echo '<span class="current">' . esc_html__( 'Blog', 'bright-autonomy' ) . '</span>';
			}
			...
			if ( $posts_page ) {
				...
				echo '<a href="' . esc_url( home_url( '/blog/' ) ) . '">' . esc_html__( 'Blog', 'bright-autonomy' ) . '</a>';
			}
			echo ' <span class="breadcrumb-separator">/</span> ';
			...
			echo '<span class="current">' . sprintf( esc_html__( 'Search results for: %s', 'bright-autonomy' ), get_search_query() ) . '</span>';

		} elseif ( is_404() ) {
			echo '<span class="current">' . esc_html__( '404 Not Found', 'bright-autonomy' ) . '</span>';
