...
if ( ! function_exists( 'bright_autonomy_render_post_card' ) ) {
	function bright_autonomy_render_post_card( $post_id = null, $args = [] ) {
		...
		$title      = $title ? $title : __( 'Untitled', 'bright-autonomy' );
		...
		<article id="post-<?php echo esc_attr( $post_id ); ?>" <?php post_class( implode( ' ', array_map( 'sanitize_html_class', $card_classes ) ), $post_id ); ?>>
			<a class="post-card__media" href="<?php echo esc_url( $permalink ); ?>" aria-label="<?php echo esc_attr( sprintf( __( 'Read %s', 'bright-autonomy' ), $title ) ); ?>">
				<?php if ( $thumbnail_data && function_exists( 'bright_autonomy_render_responsive_picture' ) ) : ?>
					<?php
				...
				<?php else : ?>
					<span class="post-card__placeholder" aria-hidden="true">
						<span><?php echo esc_html( $category ? $category->name : __( 'Article', 'bright-autonomy' ) ); ?></span>
					</span>
				<?php endif; ?>
			</a>
			...
			<div class="post-card__content">
				<div class="post-card__meta">
					...
					<span class="post-card__date-wrap">
						...
						<span><?php echo esc_html( sprintf( __( '%s min read', 'bright-autonomy' ), number_format_i18n( $read_time ) ) ); ?></span>
					</span>
				</div>
				...
				<a class="post-card__link" href="<?php echo esc_url( $permalink ); ?>" aria-hidden="true" tabindex="-1">
					<span><?php esc_html_e( 'Read article', 'bright-autonomy' ); ?></span>
