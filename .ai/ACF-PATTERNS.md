# Bright Autonomy — ACF Patterns & Helper Functions

Bright Autonomy is a clean ACF-based WordPress theme framework. Every section is created per project from the client design.
...
```php
bright_autonomy_flexible_content('cms');
// or with a specific post ID:
bright_autonomy_flexible_content('cms', $post_id);
```
...
// ✅ Guard helper calls
if ( $button && function_exists( 'bright_autonomy_render_button' ) ) {
    bright_autonomy_render_button( $button );
}
...
```php
bright_autonomy_flexible_content( string $field_name = 'cms', int|null $post_id = null )
```

```php
bright_autonomy_get_first_flexible_layout() : string|false
bright_autonomy_get_last_flexible_layout()  : string|false
bright_autonomy_has_hero_first_section()    : bool
```

Global variables available inside section templates during render:
```php
$GLOBALS['bright_autonomy_previous_layout']   // name of previous layout (string)
$GLOBALS['bright_autonomy_section_index']     // 0-based position on the page (int)
$GLOBALS['bright_autonomy_last_layout']       // name of last layout on the page (string)
```

Use for spacing/padding logic:
```php
if ( 0 === $GLOBALS['bright_autonomy_section_index'] ) {
...
```php
bright_autonomy_render_responsive_picture( array $image, array $args = [] )
```
...
bright_autonomy_render_responsive_picture( $image, [
    'size_group'     => 'hero',
    'lazy'           => false,
    'fetchpriority'  => 'high',
    'class'          => 'hero-image',
...
```php
bright_autonomy_render_button( array $button_link, array $args = [] )
```
...
```php
bright_autonomy_render_buttons( array $buttons, array $args = [] )
```
...
```php
bright_autonomy_render_icon( array|int $icon, array $args = [] )
```
...
```php
bright_autonomy_render_video( string|array $video_field, array $args = [] )
```
...
```php
// Header — logo
bright_autonomy_get_site_logo()           : array|false
bright_autonomy_render_site_logo()        : void

// Header — CTA button
bright_autonomy_get_header_button()       : array|false
bright_autonomy_render_header_button()    : void

// Footer — logo (falls back to site_logo if footer_logo not set)
bright_autonomy_get_footer_logo()         : array|false
bright_autonomy_render_footer_logo()      : void

// Footer — tagline (short description below logo, not rendered by default)
bright_autonomy_get_footer_tagline()      : string|false
bright_autonomy_render_footer_tagline()   : void

// Footer — social icons (repeater: social_icon + social_link)
bright_autonomy_get_social_medias()       : array|false
bright_autonomy_render_social_medias()    : void

// Footer — copyright text (supports {year} placeholder)
bright_autonomy_get_footer_copyright()    : string|false
bright_autonomy_render_footer_copyright() : void
```

#### Footer menu helper

```php
bright_autonomy_render_footer_menu( array $args = [] ) : void
```
...
```php
bright_autonomy_render_footer_menu( [ 'location' => 'footerMenu', 'show_title' => false ] );
```
...
// footer.php
bright_autonomy_render_footer_menu( [ 'location' => 'footerMenu',  'show_title' => true ] );
bright_autonomy_render_footer_menu( [ 'location' => 'footerMenu2', 'show_title' => true ] );
bright_autonomy_render_footer_menu( [ 'location' => 'footerMenu3', 'show_title' => true ] );
```
...
```php
bright_autonomy_breadcrumb( bool $layout_padding = false, string $margin_top = 'mt-30', string $margin_bottom = '' )
```
...
```php
bright_autonomy_render_pagination( WP_Query $query = null )
```
...
$section_classes = [ 'my-section', 'bg-bright-' . $bg ];
```
...
Grid layout classes are defined globally in `assets/css/bright-autonomy-theme-style.css`. **Never redefine grid columns inside a section's own CSS.** Use these classes directly in section templates.
...
Use `bright_autonomy_render_post_card()` inside a `.card-grid.columns-3`. Pass `fetchpriority => 'high'` for the first card (above the fold) and `variant => 'featured'` to mark it for project-specific featured styling:
...
<div class="blog-grid card-grid columns-3">
    ...
    while ( have_posts() ) :
        the_post();
        $i++;
        bright_autonomy_render_post_card( null, [
...
| Field | Type | Helper |
|---|---|---|
| `site_logo` | Image | `bright_autonomy_get_site_logo()` / `bright_autonomy_render_site_logo()` |
| `header_button` | Link | `bright_autonomy_get_header_button()` / `bright_autonomy_render_header_button()` |
| `header_button_mobile_phone` | Text | `bright_autonomy_get_header_button_mobile_phone()` |
| `footer_logo` | Image | `bright_autonomy_get_footer_logo()` / `bright_autonomy_render_footer_logo()` |
| `social_medias` | Repeater (social_icon + social_link) | `bright_autonomy_get_social_medias()` / `bright_autonomy_render_social_medias()` |
| `footer_copyright` | Text | `bright_autonomy_get_footer_copyright()` / `bright_autonomy_render_footer_copyright()` |
