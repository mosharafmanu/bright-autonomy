...
// Render
if ( $video_data && function_exists( 'bright_autonomy_render_video' ) ) {
    bright_autonomy_render_video( $video_data, [
        'behavior'        => 'autoplay',
        'autoplay'        => true,
        'muted'           => true,
        'loop'            => true,
...
```php
bright_autonomy_render_video( $video_field, $args = [] )
```
...
`wp_register_*`'d (not enqueued), and `bright_autonomy_render_video()` enqueues them
...
--bright-color-accent  /* button background */
--bright-color-light   /* button border */
```
