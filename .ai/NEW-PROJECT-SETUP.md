# Bright Autonomy — New Project Setup Script
...
Copy `bright-autonomy/` to your new project's WordPress themes directory and rename it to your project slug:

```bash
# Example
cp -r /path/to/bright-autonomy /your/wp/wp-content/themes/travelnerds-theme
```

> Do **not** run the script inside the `bright-autonomy` folder itself. Always work on a copy.
...
> A copy like `wp-content/themes/my-project/bright-autonomy/style.css` (one level too
...
> ```bash
> mv /your/wp/wp-content/themes/my-project/bright-autonomy /your/wp/wp-content/themes/travelnerds-theme
...
> (`mosharafmanu/bright-autonomy`). Committing or pushing from the copy without
...
git init
git add .
git commit -m "Initial copy from bright-autonomy starter"
```
...
| PHP Function Prefix | `travelnerds` | All `bright_autonomy_` → `travelnerds_` function names |
| CSS Token Prefix | `tn` | All `--bright-` → `--tn-` CSS custom properties and utility classes |
...
| From | To |
|---|---|
| `Bright Autonomy` | Theme Name |
| `bright-autonomy` | Theme Slug |
| `bright_autonomy_` | `{prefix}_` |
| `--bright-` | `--{css-prefix}-` |
| `bg-bright-` | `bg-{css-prefix}-` |
| `color-bright-` | `color-{css-prefix}-` |
...
| File | What changes |
|---|---|
| `style.css` `:root {}` | Token prefix via global `--bright-` replacement |
...
| `languages/bright-autonomy.pot` | Renamed to `languages/{text-domain}.pot` |
| `.ai/*.md` | All `bright_autonomy_`, `bright-autonomy`, `--bright-` references |
...
**Script says Theme Slug cannot be 'bright-autonomy'**
...
```bash
perl -pi -e 's/bright-autonomy/your-slug/g' path/to/the/file.php
```
...
git checkout .
```
If you didn't, copy the original `bright-autonomy` starter again and start fresh.
