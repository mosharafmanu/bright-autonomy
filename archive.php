...
?>
	...
	<main id="primary" class="site-main blog-archive-page">
		...
		<header class="archive-header mc-container layout-padding pt-50 pt-md-70 pt-lg-100">
			<?php
			if ( is_category() ) {
				$archive_label = __( 'Category', 'bright-autonomy' );
				$archive_title = single_cat_title( '', false );
			} elseif ( is_tag() ) {
				$archive_label = __( 'Tag', 'bright-autonomy' );
				$archive_title = single_tag_title( '', false );
			} elseif ( is_author() ) {
				$archive_label = __( 'Author', 'bright-autonomy' );
				$archive_title = esc_html( get_the_author() );
			} elseif ( is_year() ) {
				$archive_label = __( 'Year', 'bright-autonomy' );
				$archive_title = get_the_date( 'Y' );
			} elseif ( is_month() ) {
				$archive_label = __( 'Month', 'bright-autonomy' );
				$archive_title = get_the_date( 'F Y' );
			} elseif ( is_day() ) {
				$archive_label = __( 'Date', 'bright-autonomy' );
